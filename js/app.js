...
/* ─── CONTEXTUAL FIT RECOMMENDATION (by bearing family) ─── */
function getFamRec(family, typeName){
  const m = {
    bolas:            {shaft:'k5 / k6',   housing:'H7 / J7',  note:'Motores eléctricos: aro interior giratorio, carga normal. H7 es frecuente pero no universal — verifica con el fabricante.'},
    rodillos_cil:     {shaft:'k6 / m6',   housing:'H7 / K7',  note:'Alta carga radial. Puede requerir mayor interferencia en eje. Para cargas muy pesadas considerar M7.'},
    esfericos:        {shaft:'k6 / m6',   housing:'H7 / K7',  note:'Autoalineante: verifica que el alojamiento permita la autoalineación. H7 con ajuste deslizante controlado.'},
    contacto_angular: {shaft:'k5 / k6',   housing:'K6 / M6',  note:'Montaje en pares: el alojamiento requiere control axial y mayor precisión. Analizar precarga antes de usar H7.'},
    contacto_angular_2h:{shaft:'k5 / k6', housing:'K6 / M6',  note:'Doble hilera: igual que contacto angular simple. Verificar precarga.'},
    axial_bolas:      {shaft:'j6',        housing:'H8',        note:'Solo carga axial pura. Ajustes más holgados — el eje no transmite carga radial.'},
    agujas:           {shaft:'k5 / k6',   housing:'H6 / J6',  note:'Pistas de alta precisión y dureza. Alojamiento con excelente acabado superficial.'},
  };
  const r = m[family] || {shaft:'k6', housing:'H7', note:'Consultar el catálogo del fabricante para esta familia de rodamientos.'};
  return '<div class="note ok" style="margin-bottom:16px;"><span class="ni">\u{1F4A1}</span><span>'
       + '<b>Orientaci\u00F3n para ' + typeName + ':</b><br>'
       + 'Eje t\u00EDpico: <b>' + r.shaft + '</b> &nbsp;&middot;&nbsp; Alojamiento t\u00EDpico: <b>' + r.housing + '</b><br>'
       + '<span style="font-size:12px;">' + r.note + '</span>'
       + '</span></div>';
}
...
function showFicha(r,suffixes=[]){
  ...
  <div class="tab-content active" id="tc-resumen">
    <div class="ficha-grid">
      <div>
        <div class="tbl-scroll"><table class="spec-table">
  ...
  <div class="tab-content" id="tc-tolerancias">
    ${getFamRec(r.family, r.type)}
    <h3>Tolerancias de eje — ⌀${r.d} mm</h3>
    ${renderShaftTolHTML(r.d)}
    <hr class="divider">
    <h3>Tolerancias de alojamiento — ⌀${r.D} mm</h3>
    ${renderHousingTolHTML(r.D)}
    <p class="note warn" style="margin-top:14px;"><span class="ni">⚠️</span><span>
    Datos ISO 286 de referencia. El ajuste correcto depende de: tipo de carga, velocidad, temperatura
    y si el aro gira o permanece estacionario. Usa el <b>Asistente de ajustes</b> para una recomendación específica.
    </span></p>
  </div>
...
function renderShaftTolHTML(d){
  ...
  let rows=cls.map(c=>{
    ...
    return `<tr><td>${c}</td><td>${rv.ei>=0?'+':''}${rv.ei} µm</td><td>${rv.es>=0?'+':''}${rv.es} µm</td><td>${min}</td><td>${max}</td></tr>`;
  }).join('');
